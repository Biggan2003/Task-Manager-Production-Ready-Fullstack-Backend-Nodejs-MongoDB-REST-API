    ...
    try {
        ...
        res
            .cookie("refreshToken", refreshToken, {
                httpOnly: true, // JavaScript থেকে refresh token access করা বন্ধ করছি
                secure: process.env.NODE_ENV === "production", // Production-এ শুধু HTTPS connection-এর মাধ্যমে cookie পাঠাচ্ছি
                sameSite: "strict", // Cross-site request থেকে cookie protection দিচ্ছি
                maxAge: 10 * 24 * 60 * 60 * 1000, // Refresh token cookie 10 দিন valid রাখছি
                path: "/api/auth", // Cookie শুধুমাত্র authentication API routes-এর জন্য পাঠাচ্ছি
            })
            .status(200)
            .json({
                message: "Login successful", // Successful login-এর response message পাঠাচ্ছি

                accessToken, // Short-lived access token frontend-কে পাঠাচ্ছি
...
const refreshAccessToken = async (req, res) => {
    try {
        const { refreshToken } = req.cookies; // HttpOnly cookie থেকে refresh token নিচ্ছি
        ...
        res
            .cookie("refreshToken", newRefreshToken, {
                httpOnly: true, // JavaScript থেকে refresh token access করা বন্ধ করছি
                secure: process.env.NODE_ENV === "production", // Production-এ শুধু HTTPS connection ব্যবহার করছি
                sameSite: "strict", // Cross-site request থেকে cookie protection দিচ্ছি
                maxAge: 10 * 24 * 60 * 60 * 1000, // নতুন refresh token 10 দিন valid রাখছি
                path: "/api/auth", // Cookie শুধুমাত্র authentication API routes-এর জন্য পাঠাচ্ছি
            })
            .status(200)
            .json({
                message: "Access token refreshed successfully", // Successful refresh-এর message পাঠাচ্ছি
                accessToken, // নতুন access token frontend-কে পাঠাচ্ছি
            });
    ...
    try {
        ...
        if (!storedToken.revokedAt) { // Token আগে থেকেই revoked কি না check করছি
            ...
            res.clearCookie("refreshToken", {
                httpOnly: true, // একই HttpOnly cookie configuration বজায় রাখছি
                secure: process.env.NODE_ENV === "production", // Production-এ HTTPS requirement বজায় রাখছি
                sameSite: "strict", // একই-site cookie policy বজায় রাখছি
                path: "/api/auth", // Login-এর সময় যে cookie path ব্যবহার করেছি সেটিই ব্যবহার করছি
            });
        }
